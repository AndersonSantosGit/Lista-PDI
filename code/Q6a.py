...
for i in range(len(medias)):
    for j in range(len(medias)):
        if(i<>j):
            distancia = math.sqrt( np.power((medias[i]-medias[j]),2) + np.power((variancias[i]-variancias[j]),2) + np.power((curtoses[i]-curtoses[j]),2) + np.power((assimetrias[i]-assimetrias[j]),2))
            if distancia < limiar:
                if not(i in niveis):
                    niveis.append(i)
